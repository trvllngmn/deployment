...
usage()
{
  echo "usage: ./load-register-tsv.sh [register] [phase] [tsvfile relative to root] [metadata|user|all] [local|remote] [custodian] [optional data dir relative to root if not REGISTER-data]"
}
...
if [ "$DATA_TO_INCLUDE" = 'metadata' ] || [ "$DATA_TO_INCLUDE" = 'all' ]
then
